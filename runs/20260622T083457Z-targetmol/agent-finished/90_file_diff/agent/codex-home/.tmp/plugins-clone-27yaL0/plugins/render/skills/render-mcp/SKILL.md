...
```bash
claude mcp add --transport http render https://mcp.render.com/mcp --header "Authorization: Bearer <redacted>"
```
...
| Missing `Bearer` prefix in auth header | Header must be `Authorization: Bearer <redacted> |
